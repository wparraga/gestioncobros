<?php		
		if (empty($_POST['firstname2'])){
        ...
        } elseif (empty($_POST['user_king2'])){
            $errors[] = "Seleccione un perfil de usuario";
        } elseif (empty($_POST['user_status2'])){
            $errors[] = "Seleccione estado de usuario";
        } elseif (
            ...
            && !empty($_POST['user_king2'])
            && !empty($_POST['user_status2'])
          )
         {
            ...
            include ("../../../config/swee.php");
                ...
                $user_king = mysqli_real_escape_string($con,(strip_tags($_POST["user_king2"],ENT_QUOTES)));
                $user_status = mysqli_real_escape_string($con,(strip_tags($_POST["user_status2"],ENT_QUOTES)));
				
				$user_id=intval($_POST['mod_id']);
					
                    $sql = "UPDATE users SET firstname='".$firstname."', lastname='".$lastname."', user_name='".$user_name."', user_email='".$user_email."', user_king='".$user_king."', user_status='".$user_status."'
                    ...
                    if ($query_update) {
                         echo'<script>
                                swal({
                                    type: "success",
                                    title: " La cuenta ha sido modificada con éxito.",
