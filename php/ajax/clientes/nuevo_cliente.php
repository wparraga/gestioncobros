...
if(is_null($strCedula) || empty($strCedula)){
	...
	$query=mysqli_query($con, "select * from clientes where cli_cedula='".$ced_cliente."'");
	...
	if (empty($_POST['cedula'])) {
		...
		$direcciondo=mysqli_real_escape_string($con,(strip_tags($_POST["direcciondo"],ENT_QUOTES)));
		$direcciontr=mysqli_real_escape_string($con,(strip_tags($_POST["direcciontr"],ENT_QUOTES)));
		$referencia=mysqli_real_escape_string($con,(strip_tags($_POST["Referencia"],ENT_QUOTES)));
		$sql="INSERT INTO clientes (cli_cedula,cli_nombres,cli_telefono,cli_dirdomicilio,cli_dirtrabajo,cli_referencia) VALUES ('$cedula','$nombres','$telefono','$direcciondo','$direcciontr','$referencia')";
